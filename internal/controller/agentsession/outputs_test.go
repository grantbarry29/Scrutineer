...
func TestOutputResourceName_truncatesLongSessionNames(t *testing.T) {
	t.Parallel()
	name := outputResourceName("scrutineer-logs-", strings.Repeat("a", 80))
	if len(name) > 63 {
		t.Fatalf("name length = %d", len(name))
	}
	if !strings.HasPrefix(name, "scrutineer-logs-") {
...
func TestResolveArtifactPath_defaultsUnderWorkspace(t *testing.T) {
	...
	session := &scrutineerv1alpha1.AgentSession{
		Spec: scrutineerv1alpha1.AgentSessionSpec{
			Workspace: scrutineerv1alpha1.WorkspaceSpec{MountPath: "/data/ws"},
		},
	}
...
func TestResolveArtifactPath_honorsSpec(t *testing.T) {
	...
	session := &scrutineerv1alpha1.AgentSession{
		Spec: scrutineerv1alpha1.AgentSessionSpec{
			Outputs: scrutineerv1alpha1.OutputSpec{ArtifactPath: "/workspace/out"},
		},
	}
	got, err := resolveArtifactPath(session)
	if err != nil {
...
func TestAppendUniqueArtifacts(t *testing.T) {
	t.Parallel()
	dst := []scrutineerv1alpha1.ArtifactRef{{Name: "agent-logs", URI: "configmap://ns/a"}}
	got := appendUniqueArtifacts(dst, []scrutineerv1alpha1.ArtifactRef{
...
func TestHasArtifactNamed(t *testing.T) {
	t.Parallel()
	arts := []scrutineerv1alpha1.ArtifactRef{{Name: artifactNameAgentLogs}}
...
func TestOutputLabels_includeSessionRef(t *testing.T) {
	...
	if labels[scrutineerjob.LabelSessionRef] != "demo" {
