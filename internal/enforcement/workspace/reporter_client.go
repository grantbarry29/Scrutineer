...
// ReporterClient posts fs-gateway runtime evidence to the controller-owned reporter.
type ReporterClient struct {
	*reporterclient.Client
}

// NewReporterClient returns a client for POST /v1/report tagged as the fs gateway.
func NewReporterClient(baseURL, tokenPath string, httpClient *http.Client) *ReporterClient {
	return &ReporterClient{reporterclient.New(baseURL, tokenPath, enforcement.BackendFSGateway, httpClient)}
}
