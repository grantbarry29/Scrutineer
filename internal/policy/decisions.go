...
const (
	...
	mergeDecisionActor = "scrutineer-controller"
)

// BuildMergeDecisions returns merge-time policy decisions for the resolved effective policy.
func BuildMergeDecisions(resolved Resolved, now time.Time) []scrutineerv1alpha1.PolicyDecision {
	ts := metav1.NewTime(now)
	mode := resolved.Mode
	out := []scrutineerv1alpha1.PolicyDecision{{
		Time:    ts,
	...
	out = append(out, ruleListDecisions(ts, mode, "network", "allowedDomains", resolved.Rules.AllowedDomains, scrutineerv1alpha1.PolicyDecisionAllow, "AllowedDomains")...)
	...
	for i := range out {
		out[i].AssuranceLevel = scrutineerv1alpha1.EvidenceControllerComputed
	}
	return out
}

func ruleListDecisions(ts metav1.Time, mode scrutineerv1alpha1.PolicyMode, typ, rule string, targets []string, action scrutineerv1alpha1.PolicyDecisionAction, reason string) []scrutineerv1alpha1.PolicyDecision {
	if len(targets) == 0 {
		return nil
	}
	out := make([]scrutineerv1alpha1.PolicyDecision, 0, len(targets))
