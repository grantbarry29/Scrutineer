...
var _ = Describe("Live runtime tool approval", func() {
	...
	It("holds a requireHumanApproval tool call until a grant, then records a redacted approval decision", func(ctx SpecContext) {
		...
		ns := newTestNamespace("relay-e2e-tool-approval")
		...
		grantRuntimeApproval(ctx, ns, reqName, "e2e-approver@relay.test")
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(req.Status.State).To(Equal(relayv1alpha1.ApprovalStateGranted))
		}, 60*time.Second, 2*time.Second).Should(Succeed())
		Expect(getSession(ctx, key).Status.Phase).To(Equal(relayv1alpha1.PhaseRunning),
		...
		Eventually(func(g Gomega) {
			...
			var granted *relayv1alpha1.PolicyDecision
			for i := range got.Status.PolicyDecisions {
				d := &got.Status.PolicyDecisions[i]
				if d.Phase == relayv1alpha1.PolicyDecisionPhaseRuntime && d.Type == "approval" &&
					d.Action == relayv1alpha1.PolicyDecisionAllow && d.Reason == toolgateway.ReasonApprovalGranted {
					granted = d
					break
				}
			}
