...
### Task: Phase 5 · per-tool runtime approval — **design done (2026-06-21); impl slices queued**
...
**Impl slices (not started — promote one at a time):**
1. Controller runtime `ApprovalRequest` variant — `spec.trigger`/`requestId`/`scope.argDigest`; generalize the gate reconcile (decision→state→audit) without gating session phase. (`make test`)
2. Reporter approval channel — `POST /v1/approvals` + `GET /v1/approvals/{id}`, reuse TokenReview + ownership auth. (`make test`)
