...
### Task: webhook-mode envtest + live e2e for the ApprovalRequest identity webhook — **discovered 2026-06-24**

Slice 8 ships the identity-stamping webhook with thorough **unit** coverage (pure stamp logic + `Handle` patch/no-op via constructed `admission.Request`), opt-in manager wiring, generated `MutatingWebhookConfiguration`, and a cert-manager `config/webhooks` overlay that renders cleanly. **Not yet covered:** (1) an **envtest** spec that runs the webhook server (via `envtest.Environment.WebhookInstallOptions` with generated certs) and asserts a `kubectl`-style grant from user X lands `spec.decidedBy=X` regardless of the client-sent value; (2) a **live e2e** that installs cert-manager in the kind cluster, deploys `config/webhooks`, and verifies an unauthorized/forged `decidedBy` is corrected end-to-end. Both were deferred to keep slice 8 reviewable (webhook envtest mutates the shared `suite_test` setup; live e2e adds a cert-manager dependency to the kind harness). Scope: test-only; no product code changes expected. Pick up when hardening Phase 5 acceptance.
