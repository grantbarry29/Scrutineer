...
> **Status:** Design + **controller runtime variant (slice 1) + reporter approval channel (slice 2) + tool-gateway hold-and-ask (slice 3) shipped**. Defines a **mid-execution** human-approval gate: a running agent's tool/MCP call is **held** until a human grants a scoped, time-bounded approval, then allowed (or denied). It reuses the `ApprovalRequest` CRD and the approver/`allOf`/audit machinery from [`phase-5-approval-workflows.md`](phase-5-approval-workflows.md) and extends the tool-gateway + reporter contracts. Live e2e is the remaining slice. Resolves open question #4 of the approval-workflows design.
