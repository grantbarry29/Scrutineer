...
- **Today (slices 1–8 done):** the contract, mode semantics, a NetworkPolicy backend (real, enforced), and *prototype* dns-proxy/tool-gateway contracts exist; sidecars are injected with **placeholder busybox images**; evidence helpers exist but are only called in-process/tests.
- **The gap (Phase 3b, active):** no in-cluster path produces or reports runtime evidence, so `status.policyDecisions`(runtime) and `status.violations` are empty at runtime.
- **The fix:** the **runtime evidence loop** — see [`phase-3-runtime-reporter-contract.md`](phase-3-runtime-reporter-contract.md). Order: reporter mechanism design → reporter loop impl → structured events API → real dns-proxy/tool-gateway images → live network violations → file policy impl.
