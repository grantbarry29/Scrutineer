---
type: Design Doc
title: Tools-Pod Chokepoint
description: "Out-of-pod successor to the removed cooperative tool tier: a per-session tools pod executes tool calls reached only through the session Envoy and holds the credentials the agent never sees — tool policy, argument rules, and approval holds, observed and mandatory."
status: draft
tracking_issue: 76
read_when: "Future tool governance — tools pod, credential mediation, ext_authz, approval holds."
---

# Tools-Pod Chokepoint — Untamperable Tool Governance

**Scope:** the out-of-pod successor to the cooperative in-pod tool tier removed in #71: a per-session tools pod that *executes* tool calls the agent can only reach through the session's Envoy, holding the credentials the agent never sees. Restores tool policy, argument rules, and mid-execution approval holds — this time `observed` and mandatory.
...
**Tracking:** epic #76 (deliberately unscheduled; Phase 0 = elevating this draft to a full design answering the open questions below); absorbs #25 (CredentialProfile / credential mediation). The dormant `ApprovalRequest` runtime variant + reporter approval channel are live in the tree (`internal/reporter`, `internal/controller/agentsession/approval_runtime.go`); the tool/argument policy schema was removed in the #75 clean break and lives in git history (pre-#75 `api/v1alpha1/`), along with the earlier cooperative-tier designs (deleted in #74).
...
- **Evidence:** tool-call payloads are *intent, observed at the chokepoint* — recovering the intent-vs-observed signal temporarily dropped with the tier removal, at `observed` assurance, stamped via a dedicated caller class.

## Open questions (answer when scheduled)

1. Policy CRD surface: reintroduce the pre-#75 tool/argument fields (git history, pre-#75) as shipped, or reshape around the executor model (tool *catalog* + per-tool grants)?
