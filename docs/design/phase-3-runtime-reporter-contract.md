...
*(As shipped, `internal/reporter/auth.go` distinguishes **two caller classes** behind these checks: agent-trust-domain callers, via the Pod → Job → session walk above; and the session's **egress-proxy pod** — controller-owned by the AgentSession itself, with the deterministic proxy pod name and the dedicated per-session ServiceAccount matching the token. The class determines the assurance stamp below.)*

**Residual risk — agent-side evidence is self-reported, not tamper-proof.** Ownership checks bind a report to one session, but any caller inside the agent pod shares the agent's trust domain, so that channel is **cooperative**: a fully compromised agent can forge or suppress *its own* reports. To keep this honest for audit/UI consumers, the reporter stamps every decision and violation with an `assuranceLevel` (API field `EvidenceAssurance`) **derived from the authenticated caller's class, overriding any client-supplied value** — a source must never be able to self-attest a higher trust level. As shipped (Slice C, [#62](https://github.com/grantbarry29/scrutineer/issues/62); `CallerIdentity.Assurance()` in `internal/reporter/auth.go`): agent-trust-domain callers are stamped `self-reported`, and the session's out-of-pod **egress-proxy pod** — the independently-observed source this level was originally reserved for — is stamped `observed`, arriving through this **same endpoint** and distinguished by caller identity, not by route. Control-plane merge-time decisions are stamped `controller` (authoritative). The `observed` tier's integrity rests on the proxy pod's separate identity/netns ([`evidence-integrity.md`](evidence-integrity.md)), not on this contract's ownership walk alone.
