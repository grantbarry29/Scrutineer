...
- **Non-canonical IPv4 spellings are resolver-dependent (spike, Envoy 1.31 / c-ares):** hex (`0x0a.0.0.1`) and single-integer (`167772161`) authorities are treated as DNS names that **fail resolution → fail closed** (confirmed). But **leading-zero dotted forms (`010.0.0.1`) resolve as decimal** (`10.0.0.1`) and are *not* matched by the canonical regex — so they can evade a pure **deny**-list. This is the same class of gap as hostname resolution: the **robust** control is an **allow-list** (default-deny — any spelling that isn't a recognized in-range literal is denied), not a deny-list. Deny-lists here are best-effort against canonical spellings; crown-jewel ranges (cloud metadata) are additionally denied by resolved IP at the kernel backstop regardless of spelling.
