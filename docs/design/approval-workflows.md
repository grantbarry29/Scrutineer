...
4. **Per-tool runtime approval** — **shipped with the cooperative tier, now dormant.** Reuses `ApprovalRequest` (runtime variant keyed by `requestId`) and the approver/`allOf`/audit machinery; the reporter carries an approval request/lookup channel and `approval_runtime.go` resolves the human decision. The cooperative in-pod hold point was removed (#71, [`untamperable-enforcement.md`](untamperable-enforcement.md)); the hold protocol and CRD surface stay dormant and move to the out-of-pod chokepoint ([`tools-pod-chokepoint.md`](tools-pod-chokepoint.md)), where they become `observed` and mandatory.
