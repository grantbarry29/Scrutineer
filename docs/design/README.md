...
| [`artifact-export.md`](artifact-export.md) | Design (code-verified, sliced into #117–#120; epic #2): pluggable object-store export for collected session outputs — S3 backend, digests in `status.artifacts`, fallback semantics, retention posture, reuse path for future evidence export. |
