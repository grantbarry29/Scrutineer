---
type: Design Doc
title: Untamperable Enforcement
description: "The enforcement doctrine: adversarial-grade-only, the verified-or-refused lock gate, removal of the cooperative in-pod tier, and deferral of tool/file governance to out-of-pod chokepoints."
status: implemented
tracking_issue: 69
read_when: "Before any enforcement work — doctrine, sequencing, and which designs are deferred."
---
...
| Non-HTTP L4 (databases, SSH, custom TCP) needs client cooperation to traverse the proxy | Reachable *today* by tunnelling over `CONNECT` — same authority RBAC, same lock, `observed` evidence ([`../egress-non-http.md`](../egress-non-http.md), #123); only proxy/`CONNECT`-**oblivious** tools that honor no proxy config stay fail-closed | #64 node interceptor (oblivious raw L4 + *unforgeable node-observed* destination; the client-declared `CONNECT` authority is only as honest as the agent's own client) |
...
| deferred | Tools pod (epic #76), LLM gateway (epic #77), arena pod, credential mediation (#25, absorbed by #76/#77), sandboxes (#29), transparent interception (#64) | design docs / epics |
