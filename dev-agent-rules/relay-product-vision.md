...
Relay is a Kubernetes-native governance and runtime control plane for autonomous AI agents. Its purpose is secure governance and runtime control, not orchestration, task running, workflow execution, or a generic agent framework.
...
- Treat Kubernetes Jobs, Tekton, Argo Workflows, Temporal, and external orchestrators as execution backends Relay can govern, not systems Relay should replace.
- Keep Relay orchestrator-agnostic. Avoid coupling APIs or controllers permanently to Kubernetes Jobs.
...
- Keep a clear control-plane / data-plane distinction. Relay APIs and controllers declare policy and desired governance state; enforcement may happen through sidecars, gateways, eBPF agents, DNS proxies, sandboxes, or orchestrator adapters.
...
- Relay protects enterprises from unsafe autonomous behavior: prompt-injected agents, malicious or overbroad tools, credential exfiltration, unexpected network access, unsafe file writes, runaway loops, unapproved production actions, and weak auditability.
...
- Do not make agents smarter, choose prompts, optimize reasoning, or become an agent SDK. Relay governs agents regardless of implementation.
- **Evidence integrity is part of the threat model.** A governance and audit product is only trustworthy if the evidence it records cannot be forged or suppressed by the very agent it governs. Relay must be honest about its current guarantee level: today's sidecar enforcement/reporting is **cooperative** — enforcement components and the agent share a pod and ServiceAccount, so a fully compromised agent could still tamper with or starve the data plane. Adversarial-grade integrity requires isolating the data plane from the agent (separate identity/network namespace, kernel/eBPF observation the agent cannot bypass, or out-of-pod enforcement). Track the gap between *cooperative* and *adversarial* integrity explicitly; never overstate enforcement strength in docs, status, or UI.
...
- Audit and evidence are first-class outputs. Relay should explain who authorized a run, what identity acted, what policy matched, what was allowed or denied, what changed, and what runtime evidence was observed.
...
- Relay should eventually include a first-class operational UI for visibility, governance, auditability, runtime observability, approvals, and debugging autonomous AI systems.
...
- Relay should feel closer to Kubernetes, Envoy, Cilium, Istio, Vault, Boundary, and Tailscale than chatbot frameworks, prompt wrappers, or consumer AI tooling.
- Relay is evolving toward a runtime governance platform, observability platform for AI agents, secure execution control plane, and Datadog/Splunk-like product for autonomous AI systems.
...
- Work on **one narrow task or vertical slice** at a time. Task state lives in **GitHub Issues** (see `dev-agent-rules/task-management.md`); read `docs/design/` (+ component READMEs/code comments) for durable technical context and `dev-agent-rules/relay-cursor-workflow.md` for implementation rules.
- Follow the **Cursor Implementation Contract** in `dev-agent-rules/task-management.md` and `dev-agent-rules/relay-cursor-workflow.md` before coding.
- Follow **Out-of-Scope Future Work Handling** in `dev-agent-rules/relay-cursor-workflow.md`: do not silently implement adjacent future work; **every** out-of-scope item must become a **GitHub Issue** (label `agent-discovered`) in the **same session** — chat-only notes are not tracking.
...
- **Do not** turn Relay into a generic workflow engine, task runner, or agent framework.
...
A successful Relay deployment should let a platform or security team answer, for every autonomous agent run: who authorized it, what it could access, what it actually did, what was blocked, what changed, and how to reproduce or audit the decision.
